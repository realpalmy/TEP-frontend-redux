...
function ProductCard({ product }) {
  ...
  useEffect(() => {
    async function getUser() {
      ...
      setWinnerUser(users.data);
    }
    getUser();
  }, [product.winnerBid]);

  const token = useSelector((state) => state.token);
  const curUser = token?.[0].id;
        ...
        <div className="shadow card" style={{ width: "20rem" }}>
          ...
          <ul className="list-group list-group-flush">
            <li className="list-group-item">
              <div className="row">
                <div
                  className="d-flex col-12 show-high text-center text-primary align-items-center pb-2"
                  style={{ display: "flex" }}
                >
                  <div className="ms-5 me-3">Higher Bid!! : </div>
                  <div className="">{winnerUser.username}</div>
                </div>
            ...
            <li className="list-group-item">
              <div className="row">
                <div className="col-8 border-end text-center text-danger">
                  {x}
                </div>
                <div className="col-4 text-center text-success">
                  {product.bitAmount.length} bid(s)
                </div>
              </div>
            </li>
          </ul>
          ...
          <div className="card-body d-grid gap-2 col-lg-8 mx-auto text-center">
            <Link
              to={
                token
                  ...
                  : `/BidLoginPages/${product.id}`
              }
              class="text-light text-decoration-none"
            >
              <button disabled={disabled} type="submit" class={styledisable}>
                Submit A Bid
              </button>
            </Link>
            ...
            <div
              class={
                curUser == product.owner
                  ? "text-light text-decoration-none"
                  : "text-light text-decoration-none hide-block"
              }
            >
              <button
                type="submit"
                class="btn-lg bg-delete rounded-pill p-2 text-white mx-5 px-4"
                onClick={() => {
